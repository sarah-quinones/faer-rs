...
pub unsafe trait Ptr: Sized + Copy + Ord {
    type Item: Copy;
...
unsafe impl<T: Copy> Ptr for *mut T {
    type Item = T;
    ...
    F: FnMut(P, P) -> bool,
{
    ...
    unsafe {
        if is_less(v.add(1), v.add(0)) {
            ...
            //
            // 1. Swap adjacent elements until the first one gets to its final destination. However,
            //    this way we copy data around more than is necessary. If elements are big
            //    structures (costly to copy), this method will be slow.
            //
            // 2. Iterate until the right place for the first element is found. Then shift the
            //    elements succeeding it to make room for it and finally place it into the remaining
            //    hole. This is a good method.
            //
...
pub unsafe fn sort_indices<I: crate::permutation::Index, E: Entity>(
    indices: &mut [I],
    values: crate::group_helpers::SliceGroupMut<E>,
) {
    #[derive(Copy, Clone)]
    struct Wrap<E: Entity>(GroupCopyFor<E, *mut E::Unit>);

    impl<E: Entity> PartialEq for Wrap<E> {
        #[inline]
        fn eq(&self, other: &Self) -> bool {
            E::faer_first(from_copy::<E, _>(self.0)) == E::faer_first(from_copy::<E, _>(other.0))
        }
    }

    impl<E: Entity> Eq for Wrap<E> {}

    impl<E: Entity> PartialOrd for Wrap<E> {
        #[inline]
    ...
    impl<E: Entity> Ord for Wrap<E> {
        #[inline]
        fn cmp(&self, other: &Self) -> core::cmp::Ordering {
            E::faer_first(from_copy::<E, _>(self.0)).cmp(&E::faer_first(from_copy::<E, _>(other.0)))
        }
    }

    unsafe impl<E: Entity> Ptr for Wrap<E> {
        type Item = GroupCopyFor<E, E::Unit>;

        #[inline]
        fn get_ptr(ptr: *mut Self::Item) -> Self {
            Self(into_copy::<E, _>(E::faer_as_ptr(
                ptr as *mut GroupFor<E, E::Unit>,
            )))
        }

        #[inline]
        fn null() -> Self {
            Self(into_copy::<E, _>(E::faer_map(E::UNIT, |()| {
                core::ptr::null_mut::<E::Unit>()
            })))
        }

        #[inline]
        unsafe fn offset_from(self, origin: Self) -> isize {
            E::faer_first(from_copy::<E, _>(self.0))
                .offset_from(E::faer_first(from_copy::<E, _>(origin.0)) as *const _)
        }

        #[inline]
        unsafe fn add(self, offset: usize) -> Self {
            Self(into_copy::<E, _>(E::faer_map(
        ...
        unsafe fn write(self, item: Self::Item) {
            E::faer_map(
                E::faer_zip(from_copy::<E, _>(self.0), from_copy::<E, _>(item)),
                #[inline(always)]
                |(ptr, item)| *ptr = item,
            );
        }
        ...
        unsafe fn copy_nonoverlapping(src: Self, dst: Self, len: usize) {
            E::faer_map(
                E::faer_zip(from_copy::<E, _>(src.0), from_copy::<E, _>(dst.0)),
                #[inline(always)]
                |(src, dst)| core::ptr::copy_nonoverlapping(src, dst, len),
            );
        }
        #[inline]
        unsafe fn reverse(ptr: Self, len: usize) {
            E::faer_map(
                from_copy::<E, _>(ptr.0),
                #[inline(always)]
                |ptr| core::slice::from_raw_parts_mut(ptr, len).reverse(),
            );
        }
    }
    ...
    sort_unstable_by(
        (
            indices.as_mut_ptr(),
            Wrap::<E>(into_copy::<E, _>(E::faer_map(
                values.into_inner(),
...
mod tests {
    ...
    fn test_quicksort_big() {
        ...
        for i in 0..1000 {
            assert!(a_old[perm[i]] == a[i]);
            assert!(b_old[perm[i]] == b[i]);
        }
    }
}
