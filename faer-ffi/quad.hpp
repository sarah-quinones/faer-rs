...
inline Quad fabs(Quad const& a) {
    return a.x0 < 0.0 ? -a : a;
}

inline Quad abs(Quad const& a) {
    return a.x0 < 0.0 ? -a : a;
}

inline bool isfinite(Quad const& a) {
    return std::isfinite(a.x0);
}

inline bool isinf(Quad const& a) {
    return std::isinf(a.x0);
}
...
namespace std {
template<>
struct numeric_limits<quad::f128> {
    ...
    static quad::f128 min() {
        return quad::f128 {1e-200};
    }

    static quad::f128 max() {
        return 1.0 / min();
    }
