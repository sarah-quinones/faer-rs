...
fn iterate_arnoldi<T: ComplexField>(
	...
	for j in start..end + 1 {
		...
		if norm > zero() {
			...
			zip!(&mut Vnext).for_each(|unzip!(v)| *v = v.mul_real(&norm_inv));
		}
		H[j] = norm.to_cplx();
	}
}
...
fn partial_schur_real_imp<T: RealField>(
		...
		for iter in 0..restarts {
			...
			while j < n {
				...
				if w_im[j] != zero::<T>() {
					...
					let res = Hmm * re.hypot(im);
					residual[active + j] = res.copy();
	...
	while j < n {
		...
		let re = &H[(j, j)];
		if cplx {
			let im = H[(j + 1, j)].abs().sqrt() * H[(j, j + 1)].abs().sqrt();
			norms[j] = re.hypot(&im);
			norms[j + 1] = norms[j].copy();
			perm[j] = j;
			perm[j + 1] = j;
		} else {
			norms[j] = re.abs();
	...
	while idx < limit {
		...
		if cplx {
			...
			let im = H[(j + 1, j)].abs().sqrt() * H[(j, j + 1)].abs().sqrt();
			if idx + 1 < limit {
				eigvals[idx + 1] = Complex::new(re.copy(), -&im);
			}
