...
fn partial_schur_real_imp<T: RealField>(
	...
	while j < n {
		...
		if cplx {
			...
			if j + 1 < n_eigval {
				eigvals[j + 1] = Complex::new(copy(*re), -im);
			}
			eigvals[j] = Complex::new(copy(*re), im);
			if j + 1 < n_eigval {
				let (ej, ej1) = eigvecs.rb_mut().two_cols_mut(j, j + 1);
				zip!(ej, ej1, v_re, v_im).for_each(|unzip!(y0, y1, re, im)| {
					*y0 = Complex::new(copy(*re), copy(*im));
					*y1 = Complex::new(copy(*re), -*im);
				});
			} else {
				let ej = eigvecs.rb_mut().col_mut(j);
				zip!(ej, v_re, v_im).for_each(|unzip!(y0, re, im)| {
					*y0 = Complex::new(copy(*re), copy(*im));
				});
			}
		} else {
			eigvals[j] = Complex::new(copy(*re), zero());
			zip!(eigvecs.rb_mut().col_mut(j), v_re).for_each(|unzip!(y, x)| *y = Complex::new(copy(*x), zero()));
		}
...
fn partial_schur_cplx_imp<T: ComplexField>(
	...
	for j in 0..n {
		...
		eigvals[j] = Complex::new(real(*w), imag(*w));
		zip!(eigvecs.rb_mut().col_mut(j), v).for_each(|unzip!(y, x)| *y = Complex::new(real(*x), imag(*x)));
	}
	n
}
