...
pub mod supernodal {
	...
	/// representations
	#[derive(Debug, Clone)]
	pub struct SupernodalLu<I: Index, T> {
		nrows: usize,
		ncols: usize,
		nsupernodes: usize,
	...
	pub fn factorize_supernodal_numeric_lu<I: Index, T: ComplexField>(
		...
		for s in 0..n_supernodes {
			...
			lu.l_row_idx[lu.l_col_ptr_for_row_idx[s].zx()
				..lu.l_col_ptr_for_row_idx[s + 1].zx()]
				.copy_from_slice(&s_row_idx[..s_row_idx_count]);
			lu.l_row_idx[lu.l_col_ptr_for_row_idx[s].zx()
				..lu.l_col_ptr_for_row_idx[s + 1].zx()]
				.sort_unstable();
...
pub mod simplicial {
	...
	pub fn factorize_simplicial_numeric_lu<I: Index, T: ComplexField>(
		...
		for j in 0..n {
			...
			let x_piv = x[pivot_idx].copy();
			let ref x_piv_inv = x_piv.recip();
...
#[derive(Debug, Clone)]
pub struct NumericLu<I: Index, T> {
