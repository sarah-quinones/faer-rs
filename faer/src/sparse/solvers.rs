...
impl<I: Index, C: Conjugate, Inner: for<'short> Reborrow<'short, Target = csc_numeric::Ref<'short, I, C>>> csc_numeric::generic::SparseColMat<Inner> {
	...
	#[track_caller]
	pub fn sp_solve_lower_triangular_in_place(&self, mut rhs: impl AsMatMut<T = C::Canonical, Rows = usize>) {
		let this = self.rb().canonical();
		let conj = if C::IS_CANONICAL { Conj::No } else { Conj::Yes };
		linalg_sp::triangular_solve::solve_lower_triangular_in_place(this, conj, rhs.as_mat_mut().as_dyn_cols_mut(), get_global_parallelism());
	}
	...
	#[track_caller]
	pub fn sp_solve_upper_triangular_in_place(&self, mut rhs: impl AsMatMut<T = C::Canonical, Rows = usize>) {
		let this = self.rb().canonical();
		let conj = if C::IS_CANONICAL { Conj::No } else { Conj::Yes };
		linalg_sp::triangular_solve::solve_upper_triangular_in_place(this, conj, rhs.as_mat_mut().as_dyn_cols_mut(), get_global_parallelism());
	}
	...
	/// the diagonal element is assumed to be the first stored element in each column
	#[track_caller]
	pub fn sp_solve_unit_lower_triangular_in_place(&self, mut rhs: impl AsMatMut<T = C::Canonical, Rows = usize>) {
		let this = self.rb().canonical();
		let conj = if C::IS_CANONICAL { Conj::No } else { Conj::Yes };
		linalg_sp::triangular_solve::solve_unit_lower_triangular_in_place(this, conj, rhs.as_mat_mut().as_dyn_cols_mut(), get_global_parallelism());
	}
	...
	/// the diagonal element is assumed to be the last stored element in each column
	#[track_caller]
	pub fn sp_solve_unit_upper_triangular_in_place(&self, mut rhs: impl AsMatMut<T = C::Canonical, Rows = usize>) {
		let this = self.rb().canonical();
		let conj = if C::IS_CANONICAL { Conj::No } else { Conj::Yes };
		linalg_sp::triangular_solve::solve_unit_upper_triangular_in_place(this, conj, rhs.as_mat_mut().as_dyn_cols_mut(), get_global_parallelism());
	}
}
