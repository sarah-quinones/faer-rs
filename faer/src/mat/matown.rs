...
impl<T, Rows: Shape, Cols: Shape> Mat<T, Rows, Cols> {
    unsafe fn init_with(
        ...
        let mut ptr = ptr
            .wrapping_add(stride * old_ncols.unbound())
...
impl<T, Rows: Shape, Cols: Shape> Mat<T, Rows, Cols> {
    ...
    pub fn par_col_chunks(
        &self,
        chunk_size: usize,
    ) -> impl '_ + rayon::iter::IndexedParallelIterator<Item = MatRef<'_, T, Rows, usize>>
    where
        T: Sync,
    {
    ...
    pub fn par_row_chunks(
        &self,
        chunk_size: usize,
    ) -> impl '_ + rayon::iter::IndexedParallelIterator<Item = MatRef<'_, T, usize, Cols>>
    where
        T: Sync,
    {
...
impl<T, Rows: Shape, Cols: Shape> Mat<T, Rows, Cols> {
    ...
    pub fn par_col_chunks_mut(
        &mut self,
        chunk_size: usize,
    ) -> impl '_ + rayon::iter::IndexedParallelIterator<Item = MatMut<'_, T, Rows, usize>>
    where
        T: Send,
    {
    ...
    pub fn par_row_chunks_mut(
        &mut self,
        chunk_size: usize,
    ) -> impl '_ + rayon::iter::IndexedParallelIterator<Item = MatMut<'_, T, usize, Cols>>
    where
        T: Send,
    {
