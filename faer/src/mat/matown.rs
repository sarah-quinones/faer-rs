...
impl<T> RawMatUnit<T> {
	fn try_with_capacity(
		...
		let ptr = if size == 0 {
			...
			layout = StackReq::new_aligned::<u8>(
				new_layout.size(),
				new_layout.align(),
			);
...
impl<T> RawMat<T> {
	...
	fn try_with_capacity(
		...
		let alloc = {
			let alloc =
				RawMatUnit::<T>::try_with_capacity(row_capacity, col_capacity);
		...
		let row_capacity;
		let ptr = {
			let (ptr, new_row_capacity, _, unit_layout) =
				alloc.unwrap().into_raw_parts();
	...
	fn try_reserve(
		...
		if new_row_capacity > self.row_capacity
			|| new_col_capacity > self.col_capacity
		{
			self.do_reserve_with(
				nrows,
				ncols,
				new_row_capacity,
				new_col_capacity,
...
impl<T> Drop for DropCol<T> {
	...
	fn drop(&mut self) {
		if const { core::mem::needs_drop::<T>() } {
			unsafe {
				let slice =
					core::slice::from_raw_parts_mut(self.ptr, self.nrows);
...
impl<T> Mat<T> {
	...
	/// columns without reallocating. does nothing if the capacity is already
	/// sufficient
	#[track_caller]
...
impl<T, Rows: Shape, Cols: Shape> Mat<T, Rows, Cols> {
	...
	pub fn from_fn(
		...
		unsafe {
			let raw = RawMat::<T>::try_with_capacity(
				nrows.unbound(),
				ncols.unbound(),
			)
			.unwrap();
	...
	/// returns a new identity matrix, with ones on the diagonal and zeros
	/// everywhere else
	#[inline]
	...
	/// reserves the minimum capacity for `new_row_capacity` rows and
	/// `new_col_capacity` columns without reallocating, or returns an error in
	/// case of failure. does nothing if the capacity is already sufficient
	pub fn try_reserve(
		&mut self,
		new_row_capacity: usize,
		new_col_capacity: usize,
	) -> Result<(), TryReserveError> {
		self.0.raw.try_reserve(
			self.0.nrows.unbound(),
			self.0.ncols.unbound(),
			new_row_capacity,
			new_col_capacity,
		)
	}
	...
	/// reserves the minimum capacity for `new_row_capacity` rows and
	/// `new_col_capacity` columns without reallocating. does nothing if the
	...
	/// resizes the matrix in-place so that the new dimensions are `(new_nrows,
	/// new_ncols)`. new elements are created with the given function `f`, so
	/// that elements at index `(i, j)` are created by calling `f(i, j)`.
	pub fn resize_with(
		&mut self,
		new_nrows: Rows,
		new_ncols: Cols,
	...
	/// truncates the matrix so that its new dimensions are `new_nrows` and
	/// `new_ncols`. both of the new dimensions must be smaller than or equal
		...
		if new_ncols < self.0.ncols {
			...
			drop(DropMat {
				ptr: self
					...
					.ptr
					.as_ptr()
					.wrapping_add(stride * new_ncols.unbound()),
	...
	/// - the elements that were previously out of bounds but are now in bounds
	///   must be
	/// initialized
...
impl<T: core::fmt::Debug, Rows: Shape, Cols: Shape> core::fmt::Debug
	for Own<T, Rows, Cols>
{
...
impl<T, Rows: Shape, Cols: Shape> Mat<T, Rows, Cols> {
	...
	pub fn par_col_iter(
		&self,
	) -> impl '_ + rayon::iter::IndexedParallelIterator<Item = ColRef<'_, T, Rows>>
	where
		T: Sync,
	{
	...
	pub fn par_row_iter(
		&self,
	) -> impl '_ + rayon::iter::IndexedParallelIterator<Item = RowRef<'_, T, Cols>>
	where
		T: Sync,
	{
	...
	{
		<MatRef<'_, T, Rows, Cols> as MatIndex<RowRange, ColRange>>::get(
			self.as_ref(),
			row,
			col,
		)
	...
	{
		unsafe {
			<MatRef<'_, T, Rows, Cols> as MatIndex<RowRange, ColRange>>::get_unchecked(self.as_ref(), row, col)
		}
	}
	...
	{
		<MatMut<'_, T, Rows, Cols> as MatIndex<RowRange, ColRange>>::get(
			self.as_mut(),
			row,
			col,
		)
	...
	{
		unsafe {
			<MatMut<'_, T, Rows, Cols> as MatIndex<RowRange, ColRange>>::get_unchecked(self.as_mut(), row, col)
		}
	}
}
impl<T, Rows: Shape, Cols: Shape> Mat<T, Rows, Cols> {
	...
	pub fn as_dyn_stride_mut(
		&mut self,
	) -> MatMut<'_, T, Rows, Cols, isize, isize> {
	...
	pub fn par_col_iter_mut(
		&mut self,
	) -> impl '_ + rayon::iter::IndexedParallelIterator<Item = ColMut<'_, T, Rows>>
	where
		T: Send,
	{
	...
	pub fn par_row_iter_mut(
		&mut self,
	) -> impl '_ + rayon::iter::IndexedParallelIterator<Item = RowMut<'_, T, Cols>>
	where
		T: Send,
	{
	...
	pub fn two_rows_mut(
		...
		i0: Idx<Rows>,
		i1: Idx<Rows>,
	) -> (RowMut<'_, T, Cols>, RowMut<'_, T, Cols>) {
...
impl<'short, T, Rows: Shape, Cols: Shape> ReborrowMut<'short>
	for Own<T, Rows, Cols>
{
...
impl<T, Cols: Shape> Mat<T, usize, Cols> {
	...
	/// # panics
	/// The function panics if the number of columns in the row does not match
...
impl<T, Rows: Shape> Mat<T, Rows, usize> {
	...
	/// # panics
	/// The function panics if the number of rows in the col does not match the
