...
{
	type Output = <MatMut<'a, T, Rows, Cols, Rs, Cs> as MatIndex<RowRange, ColRange>>::Target;

	#[inline]
	fn index_mut(&'a mut self, (row, col): (RowRange, ColRange)) -> Self::Output {
		<MatMut<'a, T, Rows, Cols, Rs, Cs> as MatIndex<RowRange, ColRange>>::get(self.as_mut(), row, col)
	}
}
	...
	MatMut<'a, T, Rows, Cols, Rs, Cs>: MatIndex<RowRange, ColRange>,
{
	...
	fn index_move(self, (row, col): (RowRange, ColRange)) -> Self::Output {
		<MatMut<'a, T, Rows, Cols, Rs, Cs> as MatIndex<RowRange, ColRange>>::get(self, row, col)
	}
}
