...
impl<'N, T: ComplexField, S: Simd> SimdCtx<'N, T, S> {
    ...
    pub fn new_align(simd: T::SimdCtx<S>, len: Dim<'N>, align_offset: usize) -> Self {
        ...
        if align_offset == 0 {
            ...
            if align_offset <= *len {
                let head_mask = T::simd_mask_between(
                    &simd,
                    T::Index::truncate(offset),
                    T::Index::truncate(stride),
                );
                let tail_mask = T::simd_mask_between(
                    &simd,
                    T::Index::truncate(0),
                ...
                let tail_mask =
                    T::simd_mask_between(&simd, T::Index::truncate(0), T::Index::truncate(0));
