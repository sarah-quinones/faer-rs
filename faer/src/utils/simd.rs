...
pub struct SimdCtx<'N, T: ComplexField, S: Simd> {
	...
	body_end: usize,
	tail_end: usize,
	head_mask: T::SimdMemMask<S>,
	tail_mask: T::SimdMemMask<S>,
}
...
impl<'N, T: ComplexField, S: Simd> SimdIndex<'N, T, S> for SimdHead<'N, T, S> {
	#[inline(always)]
	fn read(simd: &SimdCtx<'N, T, S>, slice: ColRef<'_, T, Dim<'N>, ContiguousFwd>, index: Self) -> T::SimdVec<S> {
		unsafe { simd.mask_load(simd.head_mask, slice.as_ptr().wrapping_offset(index.start) as *const T::SimdVec<S>) }
	}
...
impl<'N, T: ComplexField, S: Simd> SimdIndex<'N, T, S> for SimdTail<'N, T, S> {
	#[inline(always)]
	fn read(simd: &SimdCtx<'N, T, S>, slice: ColRef<'_, T, Dim<'N>, ContiguousFwd>, index: Self) -> T::SimdVec<S> {
		unsafe { simd.mask_load(simd.tail_mask, slice.as_ptr().wrapping_offset(index.start) as *const T::SimdVec<S>) }
	}
...
impl<'N, T: ComplexField, S: Simd> SimdCtx<'N, T, S> {
	...
	pub fn new(simd: T::SimdCtx<S>, len: Dim<'N>) -> Self {
		...
		Self {
			...
			head_mask: T::simd_mask_between(&simd, T::Index::truncate(0), T::Index::truncate(0)),
			tail_mask: T::simd_mask_between(&simd, T::Index::truncate(0), T::Index::truncate(*len % stride)),
		}
	}
	...
	pub fn new_align(simd: T::SimdCtx<S>, len: Dim<'N>, align_offset: usize) -> Self {
		...
		if align_offset == 0 {
			...
			if align_offset <= *len {
				let head_mask = T::simd_mask_between(&simd, T::Index::truncate(offset), T::Index::truncate(stride));
				let tail_mask = T::simd_mask_between(&simd, T::Index::truncate(0), T::Index::truncate(full_len % stride));
				...
				let head_mask = T::simd_mask_between(&simd, T::Index::truncate(offset), T::Index::truncate(full_len % stride));
				let tail_mask = T::simd_mask_between(&simd, T::Index::truncate(0), T::Index::truncate(0));
	...
	pub fn new_force_mask(simd: T::SimdCtx<S>, len: Dim<'N>) -> Self {
		...
		Self {
			...
			head_mask: T::simd_mask_between(&simd, T::Index::truncate(0), T::Index::truncate(0)),
			tail_mask: T::simd_mask_between(&simd, T::Index::truncate(0), T::Index::truncate((new_len % stride) + 1)),
		}
	}
