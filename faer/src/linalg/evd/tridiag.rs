        ...
        for k in 0..n {
            ...
            let tau_inv;
            {
...

                            {
                                ...
                                z2.rb_mut().par_col_iter_mut().enumerate().for_each(
                                    |(idx, mut z2)| {
                                        ...
                                        {
                                            let y2 =
                                                unsafe { y2.subrows(first, ncols).const_cast() };
                ...
                *y1 = mul_real(
                ...
                let b = mul_real(
                    mul_pow2(
                        *y1 + dot::inner_prod(x2.rb().transpose(), Conj::Yes, y2.rb(), Conj::No),
                        from_f64(0.5),
                    ),
                    tau_inv,
                );
    ...
    fn test_tridiag_cplx() {
        ...
        for n in [2, 3, 4, 8, 16] {
            ...
            tridiag_in_place(
                ...
                Par::Seq,
                DynStack::new(&mut GlobalMemBuffer::new(
                    tridiag_in_place_scratch::<c64>(n, Par::Seq, auto!(c64)).unwrap(),
                )),
                auto!(c64),
            );
