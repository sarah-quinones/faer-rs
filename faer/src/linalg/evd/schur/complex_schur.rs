	...
	for iter in 0..itmax {
		...
		for i in (istart + 1..istop).rev() {
			...
			if A[(i, i - 1)].abs1() <= eps * tst {
				...
				let ref aa = A[(i, i)].abs1().fmax((&A[(i, i)] - &A[(i - 1, i - 1)]).abs1());

				let ref bb = A[(i, i)].abs1().fmin((&A[(i, i)] - &A[(i - 1, i - 1)]).abs1());
		...
		if (&s1 - &A[(istop - 1, istop - 1)]).abs1() > (&s2 - &A[(istop - 1, istop - 1)]).abs1() {
			s1 = s2.copy();
		}
		...
		if istart + 2 < istop {
			for i in (istart + 1..istop - 1).rev() {
				...
				if (sn.conj() * &A[(i, i - 1)]).abs1() <= eps * (A[(i, i - 1)].abs1() + A[(i, i + 1)].abs1()) {
					istart2 = i;
		...
		for i in istart2..istop - 1 {
			...
			if i == istart2 {
				...
				(rot, r) = JacobiRotation::rotg(A[(i, i - 1)].copy(), A[(i + 1, i - 1)].copy());
...
fn lahqr_shiftcolumn<T: ComplexField>(h: MatRef<'_, T>, v: ColMut<'_, T>, s1: T, s2: T) {
	...
	if n == 2 {
		...
		if s == zero() {
			...
			v[0] = h10s * &h[(0, 1)] + (&h[(0, 0)] - s1) * (&h[(0, 0)] - s2).mul_real(s_inv);
		...
		if s == zero() {
			...
			v[0] = (&h[(0, 0)] - s1) * (&h[(0, 0)] - s2).mul_real(s_inv) + &h[(0, 1)] * h10s + &h[(0, 2)] * h20s;
...
fn introduce_bulges<T: ComplexField>(
	...
	for i_pos_last in ilo..ilo + n_block - 2 {
		...
		for i_bulge in 0..n_active_bulges {
			...
			let ref sum = &a[(i_pos, i_pos)] + v1.conj() * &a[(i_pos + 1, i_pos)] + v2.conj() * &a[(i_pos + 2, i_pos)];
		...
		for i_bulge in 0..n_active_bulges {
			...
			for j in i_pos + 1..istop_m {
				let ref sum = &a[(i_pos, j)] + v1.conj() * &a[(i_pos + 1, j)] + v2.conj() * &a[(i_pos + 2, j)];
		...
		for i_pos_last in *i_pos_block + n_shifts - 2..*i_pos_block + n_shifts - 2 + n_pos {
			...
			for i_bulge in 0..n_bulges {
				...
				for j in i1..i2 {
					...
					u2[(j, i_pos - *i_pos_block + 1)] -= sum.mul_real(v0) * v1.conj();

					u2[(j, i_pos - *i_pos_block + 2)] -= sum.mul_real(v0) * v2.conj();
				}
			}
		}
...
fn remove_bulges<T: ComplexField>(
	...
	let ref small_num = min_positive::<T::Real>() / eps * from_f64::<T::Real>(n as f64);
		...
		for i_bulge in i_bulge_start..n_bulges {
			...
			if i_pos == ihi - 2 {
				...
				for j in i_pos..istop_m {
					let ref sum = &a[(i_pos, j)] + (v1.conj() * &a[(i_pos + 1, j)]);
...
mod tests {
	...
	fn test_3() {
		...
		let h = MatRef::from_row_major_array(try_const! {
			& [[c64::new(0.997386, 0.677592), c64::new(0.646064, 0.936948),
			c64::new(0.090948, 0.674011)], [c64::new(0.212396, 0.976794),
			c64::new(0.460270, 0.926436), c64::new(0.494441, 0.888187)],
			[c64::new(0.000000, 0.000000), c64::new(0.616652, 0.840012),
