...
pub fn reconstruct_scratch<I: Index, T: ComplexField>(dim: usize, par: Par) -> Result<StackReq, SizeOverflow> {
	_ = par;
...
mod tests {
	...
	fn test_reconstruct() {
		...
		let (_, perm) = factor::cholesky_in_place(
			...
			perm_bwd,
			Par::Seq,
			DynStack::new(&mut { GlobalMemBuffer::new(factor::cholesky_in_place_scratch::<usize, c64>(n, Par::Seq, auto!(c64)).unwrap()) }),
			auto!(c64),
		);
