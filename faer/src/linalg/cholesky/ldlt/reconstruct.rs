...
#[track_caller]
#[math]
pub fn reconstruct<T: ComplexField>(out: MatMut<'_, T>, L: MatRef<'_, T>, D: DiagRef<'_, T>, par: Par, stack: &mut DynStack) {
	let mut out = out;
	_ = stack;
...
mod tests {
	...
	fn test_reconstruct() {
		...
		factor::cholesky_in_place(
			...
			Default::default(),
			Par::Seq,
			DynStack::new(&mut { GlobalMemBuffer::new(factor::cholesky_in_place_scratch::<c64>(n, Par::Seq, auto!(c64)).unwrap()) }),
			auto!(c64),
		)
		.unwrap();
