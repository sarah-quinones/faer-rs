...
pub(crate) fn rank_update_indices(start_col: usize, indices: &[usize]) -> impl FnMut() -> usize + '_ {
...
pub(crate) fn delete_rows_and_cols_triangular<T: ComplexField>(A: MatMut<'_, T>, idx: &[usize]) {
	let mut A = A;
	let n = A.nrows();
	let r = idx.len();
	let r1 = r + 1;
...
#[track_caller]
pub fn rank_r_update_clobber<T: ComplexField>(LD: MatMut<'_, T>, w: MatMut<'_, T>, alpha: DiagMut<'_, T>) {
	let n = LD.nrows();
	let r = w.ncols();
	assert!(all(LD.nrows() == n, LD.ncols() == n, w.nrows() == n, w.ncols() == r, alpha.dim() == r));
	if n == 0 {
		return;
	}
	RankRUpdate {
...
#[track_caller]
pub fn delete_rows_and_cols_clobber_scratch<T: ComplexField>(dim: usize, n_removed: usize) -> StackReq {
	temp_mat_scratch::<T>(dim, n_removed).and(temp_mat_scratch::<T>(n_removed, 1))
}
#[track_caller]
pub fn insert_rows_and_cols_clobber_scratch<T: ComplexField>(n_inserted: usize, par: Par) -> StackReq {
	super::factor::cholesky_in_place_scratch::<T>(n_inserted, par, default())
}
#[track_caller]
pub fn delete_rows_and_cols_clobber<T: ComplexField>(LD: MatMut<'_, T>, indices: &mut [usize], par: Par, stack: &mut MemStack) {
	let n = LD.nrows();
	let r = indices.len();
	_ = par;
