...
#[inline(always)]
fn norm_l2_sqr_simd<'N, T: ComplexField>(
		...
		if const { T::SIMD_CAPABILITIES.is_simd() } {
			if let Some(mat) = mat.try_as_col_major() {
				if const { T::IS_NATIVE_C32 } {
					...
					return unsafe {
						crate::hacks::coerce(norm_l2_sqr_simd_pairwise_cols::<
							f32,
						>(mat))
					};
...
mod tests {
	...
	fn test_norm_l2_sqr() {
		...
		for (m, n) in [(9, 10), (1023, 5), (42, 1)] {
			for factor in [0.0, 1.0, 1e30, 1e120, 1e-30, 1e-250] {
				...
				if target == 0.0 {
					...
					assert!(
						relative_err(norm_l2_sqr(mat.as_ref()), target) < 1e-14
					);
				}
			}
		}
		...
		assert!(
			relative_err(norm_l2_sqr(mat.as_ref().as_mat()), target) < 1e-14
		);
	}
}
