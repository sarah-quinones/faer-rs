...
#[inline(always)]
#[math]
fn norm_max_simd<'N, C: ComplexContainer, T: ComplexField<C>>(
    ctx: &Ctx<C, T>,
    ...
    impl<'N, C: ComplexContainer, T: ComplexField<C>> pulp::WithSimd for Impl<'_, 'N, C, T> {
        ...
        fn with_simd<S: pulp::Simd>(self, simd: S) -> Self::Output {
            ...
            for [i0, i1, i2, i3] in head {
                let x0 = simd.abs_max(simd.read(rb!(data), i0));
                let x1 = simd.abs_max(simd.read(rb!(data), i1));
                let x2 = simd.abs_max(simd.read(rb!(data), i2));
                let x3 = simd.abs_max(simd.read(rb!(data), i3));
            ...
            if simd.has_tail() {
                let x0 = simd.abs_max(simd.read_tail(rb!(data)));
                acc0 = simd.max(acc0, x0);
            }
...
#[math]
fn norm_max_simd_pairwise_rows<C: ComplexContainer, T: ComplexField<C>>(
    ctx: &Ctx<C, T>,
    ...
    if data.nrows() <= LINEAR_IMPL_THRESHOLD {
        ...
        norm_max_simd(ctx, data.as_row_shape(N))
    } else {
        let split_point = ((data.nrows() + 1) / 2).next_power_of_two();
        let (head, tail) = data.split_at_row(split_point);
        let acc0 = norm_max_simd_pairwise_rows(ctx, head);
        let acc1 = norm_max_simd_pairwise_rows(ctx, tail);
...
#[math]
fn norm_max_simd_pairwise_cols<C: ComplexContainer, T: ComplexField<C>>(
    ctx: &Ctx<C, T>,
    data: MatRef<'_, C, T, usize, usize, ContiguousFwd>,
) -> <C::Real as Container>::Of<T::RealUnit> {
    if data.ncols() == 1 {
        norm_max_simd_pairwise_rows(ctx, data.col(0))
    } else {
        let split_point = ((data.ncols() + 1) / 2).next_power_of_two();
        let (head, tail) = data.split_at_col(split_point);
        let acc0 = norm_max_simd_pairwise_cols(ctx, head);
        let acc1 = norm_max_simd_pairwise_cols(ctx, tail);
...
#[math]
pub fn norm_max<C: ComplexContainer, T: ComplexField<C>>(
    ctx: &Ctx<C, T>,
        ...
        if const { T::SIMD_CAPABILITIES.is_simd() } {
            if let Some(mat) = mat.try_as_col_major() {
                if const { T::IS_NATIVE_C32 } {
                    ...
                    return unsafe {
                        crate::hacks::coerce(norm_max_simd_pairwise_cols::<Unit, f32>(
                            &Ctx(Unit),
                            mat,
                        ))
                    };
                    ...
                    return unsafe {
                        crate::hacks::coerce(norm_max_simd_pairwise_cols::<Unit, f64>(
                            &Ctx(Unit),
                            mat,
                        ))
                    };
                    ...
                    return math.max(
                        norm_max_simd_pairwise_cols::<C::Real, T::RealUnit>(Ctx::new(&**ctx), re),
                        norm_max_simd_pairwise_cols::<C::Real, T::RealUnit>(Ctx::new(&**ctx), im),
                    );
                } else {
                    return norm_max_simd_pairwise_cols(ctx, mat);
                }
            }
        }
        ...
        for j in 0..n {
            for i in 0..m {
                ...
                acc = math.re(max(
                    //
                    acc,
                    max(abs1(cx.real(val)), abs1(cx.imag(val))),
                ));
            }
        }
        acc
...
mod tests {
    use super::*;
    use crate::{assert, unzipped, zipped, Mat};
    ...
    fn test_norm_max() {
        ...
        for (m, n) in [(9, 10), (1023, 5), (42, 1)] {
            for factor in [0.0, 1.0, 1e30, 1e250, 1e-30, 1e-250] {
                ...
                if factor == 0.0 {
                    assert!(norm_max(&default(), mat.as_ref()) == target);
                } else {
                    assert!(relative_err(norm_max(&default(), mat.as_ref()), target) < 1e-14);
                }
            }
        }
    }
