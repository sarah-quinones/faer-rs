...
pub fn bidiag_in_place<T: ComplexField>(
    ...
    let A = A.as_dyn_mut();
    let H_left = H_left.as_dyn_mut();
    let H_right = H_right.as_dyn_mut();
    ...
    while j < size {
        ...
        let mut Hl = Hl.rb_mut().submatrix_mut(0, j, bl, bl);
        for k in 0..bl {
            Hl[(k, k)] = copy(Hl[(0, k)]);
        }
    ...
    if size > 0 {
        let n = n - 1;
        let size = size - 1;
        let A = A.rb().submatrix(0, 1, size, n);

        let mut Hr = Hr.rb_mut().subcols_mut(0, size);

        let mut j = 0;
        while j < size {
            let br = Ord::min(br, size - j);

            let mut Hr = Hr.rb_mut().submatrix_mut(0, j, br, br);
            ...
            householder::upgrade_householder_factor(
                Hr.rb_mut(),
                A.transpose().submatrix(j, j, n - j, br),
                br,
                1,
                par,
            );
...
mod tests {
    ...
    fn test_bidiag_real() {
        ...
        for (m, n) in [(8, 4), (8, 8)] {
            ...
            let size = size - 1;
            let V = UV.rb().submatrix(0, 1, size, size);
            let A1 = A.rb_mut().subcols_mut(1, size);
            let Hr = Hr.as_ref();
