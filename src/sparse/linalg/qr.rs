...
pub mod supernodal {
    ...
    fn ghost_factorize_supernodal_householder_symbolic<'m, 'n, I: Index>(
        ...
        ghost::with_size(n_supernodes, |N_SUPERNODES| {
            ...
            for (s, ((next_row_ptr, next_val_ptr), next_tau_ptr)) in zip(
                ...
                let blocksize = crate::linalg::qr::no_pivoting::compute::recommended_blocksize::<
                    Symbolic,
            ...
            E: ComplexField,
        {
            ...
            // x <- Q^T x
            {
                ...
                for s in 0..n_supernodes {
                    ...
                    let s_H = crate::mat::from_column_major_slice::<'_, E>(
                        s_H.into_inner(),
                    ...
                    let max_blocksize = H_symbolic.max_blocksize[s].zx();
                    let s_tau = crate::mat::from_column_major_slice::<'_, E>(
                        s_tau.into_inner(),
                        max_blocksize,
    ...
    pub(crate) fn factorize_supernodal_numeric_qr_impl<I: Index, E: ComplexField>(
        ...
        for s in 0..n_supernodes {
            ...
            let mut s_H = crate::mat::from_column_major_slice_mut::<'_, E>(
                s_H.into_inner(),
                s_h_row_full_end - s_h_row_begin,
            ...
            while child_ >= I::Signed::truncate(0) {
                ...
                let c_H = crate::mat::from_column_major_slice::<'_, E>(
                    c_H.into_inner(),
...

            {
                ...
                let max_blocksize = H_symbolic.max_blocksize[s].zx();
                let mut s_tau = crate::mat::from_column_major_slice_mut::<'_, E>(
                    s_tau.into_inner(),
                    max_blocksize,
                    Ord::min(s_H.ncols(), s_h_row_full_end - s_h_row_begin),
                );
...
pub mod simplicial {
    ...
    pub fn factorize_simplicial_numeric_qr_unsorted<'a, I: Index, E: ComplexField>(
        ...
        for j in 0..n {
            ...
            let mut h_col = crate::col::from_slice_mut::<E>(
                householder_values
                    .rb_mut()
