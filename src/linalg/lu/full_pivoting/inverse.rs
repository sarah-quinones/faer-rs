...
fn invert_impl<I: Index, E: ComplexField>(
    ...
    join_raw(
        |parallelism| {
            crate::linalg::triangular_inverse::invert_unit_lower_triangular(
                l_inv,
                lu_factors,
                parallelism,
            )
        ...
        |parallelism| {
            crate::linalg::triangular_inverse::invert_upper_triangular(
                u_inv,
                lu_factors,
                parallelism,
            )
    ...
    let col_p = row_perm.arrays().1;
    let row_p = col_perm.arrays().1;
    assert!(row_p.len() == n);
    assert!(col_p.len() == n);
    unsafe {
